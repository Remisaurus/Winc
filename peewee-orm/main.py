...
'''
filefortests.create_tables()
db.create_tables([DishIngredient])
...
def cheapest_dish() -> models.Dish:
    cheapest = models.Dish.select().where(models.Dish.price_in_cents == \
        models.Dish.select(fn.MIN(models.Dish.price_in_cents)).scalar())
    for cheapdish in cheapest:
        print(f'the cheapest dish seems to be:"{cheapdish.name}". It costs:"{cheapdish.price_in_cents} cents"')
    return cheapest
cheapest_dish()
